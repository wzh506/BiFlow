...
class Attention(torch.nn.Module):
    ...
    def forward_spda(
        self, x: torch.Tensor, mask: Union[torch.Tensor, None] = None, temp: float = 1.0, which_cache: str = 'cond', is_causal: bool = True
    ) -> torch.Tensor:
        B, T, C = x.size()
        x = self.norm(x.float()).type(x.dtype)
        q, k, v = self.qkv(x).reshape(B, T, 3 * self.num_heads, -1).transpose(1, 2).chunk(3, dim=1)  # (b, h, t, d)

        if self.sample and is_causal: #采样的时候才开启，存储前面历史的k,v,避免重复计算
        ...
        if not is_causal:
            mask = None  #非因果的时候不需要mask，说明这个是reverse model环节,强行去除Mask
    ...
    def forward(
        ...
        if self.USE_SPDA:
            return self.forward_spda(x, mask, temp, which_cache,is_causal)
...
class AttentionBlock(torch.nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, attn_mask: Union[torch.Tensor, None] = None, attn_temp: float = 1.0, which_cache: str = 'cond',is_causal:bool = True
    ) -> torch.Tensor:
        x = x + self.attention(x, attn_mask, attn_temp, which_cache,is_causal=is_causal)
        x = x + self.mlp(x)
        return x
...
class StudentBlock(torch.nn.Module):
    """
    BiFlow 的 Reverse Model 使用的 Block。
    它不进行 Flow 变换，只是单纯的非因果 Transformer 层。
    """
    def __init__(self, channels: int, head_dim: int, expansion: int = 4):
        super().__init__()
        # 使用和 Teacher 一样的配置，但通常不需要 MetaBlock 的复杂逻辑
        self.block = AttentionBlock(channels, head_dim, expansion)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # 关键：is_causal=False，实现并行处理
        # attn_mask=None，因为是全图可见
        return self.block(x, attn_mask=None, is_causal=False)
...
class BiFlow(torch.nn.Module):
    def __init__(self, teacher_model: TarFlowModel):
        ...
        # 注意：需要根据 meta_block 实现获取正确的 in/out features
        # 这里假设 proj_in 是 MetaBlock 的第一个层
        self.in_channels = teacher_model.blocks[0].proj_in.in_features 
        self.hidden_dim = teacher_model.blocks[0].proj_in.out_features
        
        # --- 构建 Student (Reverse Model) ---
        # 1. Input Projection (z -> hidden)
        self.student_proj_in = torch.nn.Linear(self.in_channels, self.hidden_dim)
        # 初始化 Pos Embed
        self.student_pos_embed = torch.nn.Parameter(torch.randn_like(teacher_model.blocks[0].pos_embed) * 0.02)
        
        # 2. Student Blocks (与 Teacher 层数对应)
        ...
        # 3. Hidden Alignment Projection Heads (用于对齐特征)
        self.proj_heads = torch.nn.ModuleList([
            torch.nn.Linear(self.hidden_dim, self.hidden_dim // 4)  #发现原来差了4倍
            for _ in range(num_blocks)
        ])
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor = None):
        ...
        loss_align = 0
        # Teacher 的 outputs 顺序是 [layer1_out, layer2_out, ..., z]
        # Student 是从 z 开始还原。
